...
public class StudentService {
    ...
    @Transactional
    public TutorResponseDTO updateTutor(int studentId, int teacherId) {
        log.info("Updating tutor student-id - {}, teacher-id - {}", studentId, teacherId);

        // find entities
        var studentOptional = repository.findById(studentId);
        var teacherOptional = professorRepository.findById(teacherId);
    ...
    public List<StudentResponseDTO> getStudentByTeacherId(int teacherId) {
        log.info("Getting students by teacher id - {}", teacherId);

        var teacherOptional = professorRepository.findById(teacherId);
        var teacher = teacherOptional.orElseThrow(() -> new EntityNotFoundException("Teacher not found"));

        List<Student> listOfEntities = repository.findStudentByTeacherId(teacher);
